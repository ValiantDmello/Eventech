...
from ..models import Event, EventechUser, EventAttendee, EventRequest, Feedback
import json

class AttendeeViewSet(viewsets.ModelViewSet):
    serializer_class = AttendeeSerializer
    queryset = EventechUser.objects.filter(isAdmin=False)
...
class RegisterEventView(APIView):
    def post(self, request):
        ...
        attendee = EventechUser.objects.get(id=attendee_id)
...
class SwapEventView(APIView):
    def post(self, request):
        ...
        event = Event.objects.get(id=event_id)
        attendee = EventechUser.objects.get(id=attendee_id)
        new_attendee = EventechUser.objects.get(id=new_attendee_id)
...
class AdminAttendeeAPIView(APIView):
    def get(self, request, admin_id):
        try:
            admin = EventechUser.objects.get(id=admin_id)
        except EventechUser.DoesNotExist:
...
class AdminUserList(APIView):
    ...
    def get(self, request):
        admins = EventechUser.objects.filter(isAdmin=True)
...
class FeedbackList(APIView):
    ...
    def post(self, request, event_id):
        ...
        attendee = get_object_or_404(EventechUser, id=data.get('attendee'))
